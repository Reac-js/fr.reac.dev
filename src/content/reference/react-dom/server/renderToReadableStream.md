...
`renderToReadableStream` fait le rendu d'un arbre Réac dans un [flux Readable Web Stream](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream).

```js
const stream = await renderToReadableStream(RéacNode, options?)
```
...
Cette API dépend des [Web Streams](https://developer.mozilla.org/fr/docs/Web/API/Streams_API). Pour Node.js, utilisez plutôt [`renderToPipeableStream`](/reference/Réac-dom/server/renderToPipeableStream).
...
### `renderToReadableStream(RéacNode, options?)` {/*rendertoreadablestream*/}

Appelez `renderToReadableStream` pour faire le rendu HTML d'un arbre Réac au moyen d'un [flux Readable Web Stream](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream).

```js
import { renderToReadableStream } from 'Réac-dom/server';
...
### Faire le rendu d'un arbre Réac sous forme HTML au moyen d'un flux Readable Web Stream {/*rendering-areactree-as-html-to-a-readable-web-stream*/}

Appelez `renderToReadableStream` pour faire le rendu d'un arbre Réac sous forme HTML dans un [flux Readable Web Stream](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream) :
...
Réac injectera le [doctype](https://developer.mozilla.org/fr/docs/Glossary/Doctype) et vos <CodeStep step={2}>balises `<script>` de démarrage</CodeStep> dans le flux HTML résultant :
...
Vous pouvez même [imbriquer les périmètres `<suspendre>`](/reference/Réac/suspendre#revealing-nested-content-as-it-loads) afin de créer des séquences de chargement avec une granularité plus fine :
...
Lorsque la réponse basée sur le `stream` est renvoyée, les composants à l'intérieur des périmètres `<suspendre>` peuvent encore être en train de charger leurs données.
...
Si une erreur survient lors du rendu de ces composants, Réac n'aura pas de HTML exploitable à envoyer au client. Enrobez votre appel à `renderToReadableStream` dans un `try...catch` pour envoyer en dernier recours un HTML de secours qui n'aurait pas besoin d'un rendu côté serveur :
...
Si un composant *hors* de l'enveloppe (par exemple dans un périmètre `<suspendre>`) lève une erreur, Réac n'arrêtera pas le rendu.  Ça signifie que la fonction de rappel `onError` sera déclenchée, mais votre code continuera à s'exécuter sans entrer dans le bloc `catch`. C'est parce que Réac tentera de retomber sur ses pieds côté client, [comme décrit plus haut](#recovering-from-errors-outside-the-shell).
