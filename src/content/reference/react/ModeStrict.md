...
`<ModeStrict>` vous permet de détecter des bugs courants dans vos composants pendant la phase de développement.
...
Utilisez `ModeStrict` pour activer des comportements de développement et des avertissements supplémentaires pour l'arbre des composants que vous placez à l'intérieur :
...
`ModeStrict` ne prend aucune prop.

#### Limitations {/*caveats*/}

* Il n'est pas possible de désactiver le Mode Strict au sein d'un arbre enrobé dans un `<ModeStrict>`. Ça vous garantit que tous les composants à l'intérieur de `<ModeStrict>` sont vérifiés. Si deux équipes travaillant sur un produit ne sont pas d'accord sur l'utilité de ces vérifications, elles doivent trouver un consensus ou déplacer le `<ModeStrict>` plus bas dans l'arbre.
...
Le Mode Strict active des vérifications supplémentaires uniquement en mode de développement pour tout l'arbre des composants à l'intérieur du composant `<ModeStrict>`. Ces vérifications vous aident à trouver des bugs courants dans vos composants dès le début de la phase de développement.


Pour activer le Mode Strict pour toute votre appli, enrobez votre composant racine avec `<ModeStrict>` lorsque vous en faites le rendu :
...
Nous recommandons d'enrober toute votre appli dans le Mode Strict, en particulier pour les nouvelles applis. Si vous utilisez un framework qui appelle [`createRoot`](/reference/Réac-dom/client/createRoot) à votre place, consultez sa documentation pour savoir comment activer le Mode Strict.
...
[Réac part du principe que chaque composant que vous écrivez est une fonction pure](/learn/keeping-composants-pure). Ça signifie que vos composants Réac doivent toujours renvoyer le même JSX pour les mêmes entrées (props, état et contexte).
...
- Les fonctions que vous passez à [`utiliserEtat`](/reference/Réac/utiliserEtat), aux [fonctions `set`](/reference/Réac/utiliserEtat#setstate), à [`utiliserMemoire`](/reference/Réac/utiliserMemoire) ou à [`utiliserReducteur`](/reference/Réac/utiliserReducteur).
- Certaines méthodes des composants à bases de classes comme [`constructor`](/reference/Réac/Composant#constructor), [`render`](/reference/Réac/Composant#render), [`shouldComposantUpdate`](/reference/Réac/Composant#shouldComposantupdate) ([voir la liste complète](https://Réacjs.org/docs/strict-mode.html#detecting-unexpected-side-effects)).
...
export default function StoryTray({ stories }) {
  const [isHover, setIsHover] = utiliserEtat(false);
  const items = stories;
  items.push({ id: 'create', label: 'Créer une histoire' });
  return (
    <ul
...
Ça rendrait [la fonction `StoryTray` pure](/learn/keeping-composants-pure). À chaque appel, elle ne modifierait que la copie du tableau et n'affecterait aucun objet ou variable externe. Ça résout le bug, mais vous avez dû faire en sorte que le composant fasse plus souvent son rendu pour mettre en évidence le souci dans son comportement.

**Dans cet exemple, le bug ne sautait pas aux yeux. Enrobons maintenant le code original (avec son bug) dans un composant `<ModeStrict>` :**
...
[Apprenez-en davantage sur la façon de garder les composants purs](/learn/keeping-composants-pure).

<Note>

Si vous avez installé [les outils de développement Réac](/learn/Réac-developer-tools), tous les appels à `console.log` lors du second appel apparaîtront légèrement estompés. Ces outils proposent également un paramètre (désactivé par défaut) pour les masquer complètement.
...
Chaque Effet a du code d'initialisation et peut avoir du code de nettoyage. Normalement, Réac appelle le code d'initialisation quand le composant *est monté* (quand il est ajouté à l'écran), et appelle le code de nettoyage quand le composant est *démonté* (il est enlevé de l'écran). Réac appelle ensuite le nettoyage et l'initialisation à nouveau si l'une des dépendances de l'Effet a changé depuis le dernier rendu.

Quand le Mode Strict est activé, Réac exécutera **un cycle d'initialisation + nettoyage supplémentaire en développement pour chaque Effet**.  Ça peut surprendre, mais ça aide à détecter des bugs subtils qu'il est difficile de repérer manuellement.
...
**Dans l'exemple original, le bug ne sautait pas aux yeux. Enrobez maintenant le code original (et buggué) dans un composant `<ModeStrict>` :**
...
Réac alerte si certains composants quelque part à l'intérieur de l'arbre de `<ModeStrict>` utilisent l'une de ces API dépréciées :

* [`findDOMNode`](/reference/Réac-dom/findDOMNode). [Voir les alternatives](/reference/Réac-dom/findDOMNode#alternatives).
* Les méthodes de cycle de vie `UNSAFE_` des composants à base de classes, telles que [`UNSAFE_ComposantWillMount`](/reference/Réac/Composant#unsafe_composantwillmount). [Voir les alternatives](https://legacy.Réacjs.org/blog/2018/03/27/update-on-async-rendering.html#migrating-from-legacy-lifecycles).
* Les anciens contextes ([`childContextTypes`](/reference/Réac/Composant#static-childcontexttypes), [`contextTypes`](/reference/Réac/Composant#static-contexttypes) et [`getChildContext`](/reference/Réac/Composant#getchildcontext)). [Voir les alternatives](/reference/Réac/creerContexte).
* Les anciennes refs textuelles ([`this.refs`](/reference/Réac/Composant#refs)). [Voir les alternatives](https://legacy.Réacjs.org/docs/strict-mode.html#warning-about-legacy-string-ref-api-usage).

Ces API sont avant tout utilisées dans les anciens [composants à base de classes](/reference/Réac/Composant) et n'apparaissent que rarement dans les applis modernes.
