---
title: utiliserReducteur
---

<Intro>

`utiliserReducteur` est un Hook Réac qui vous permet d'ajouter un [réducteur](/learn/extracting-state-logic-into-a-reducer) à votre composant.

```js
const [state, dispatch] = utiliserReducteur(reducer, initialArg, init?)
```
...
### `utiliserReducteur(reducer, initialArg, init?)` {/*usereducer*/}

Appelez `utiliserReducteur` au niveau racine de votre composant pour gérer son état avec un [réducteur](/learn/extracting-state-logic-into-a-reducer).
...
function MyComposant() {
  const [state, dispatch] = utiliserReducteur(reducer, { age: 42 });
  // ...
```
...
`utiliserReducteur` renvoie un tableau avec exactement deux valeurs :
...
* `utiliserReducteur` est un Hook, vous ne pouvez donc l'appeler **qu'au niveau racine de votre composant** ou dans vos propres Crochets. Vous ne pouvez pas l'appeler dans des boucles ou des conditions. Si vous avez besoin de le faire, extrayez un nouveau composant et déplacez-y l'état.
* En Mode Strict, Réac **appellera deux fois votre réducteur et votre fonction d'initialisation** afin de [vous aider à détecter des impuretés accidentelles](#my-reducer-or-initializer-function-runs-twice). Ce comportement est limité au développement et n'affecte pas la production. Si votre réducteur et votre fonction d'initialisation sont pures (ce qui devrait être le cas), ça n'impactera pas votre logique. Le résultat de l'un des appels est ignoré.

---

### Fonction `dispatch` {/*dispatch*/}

La fonction `dispatch` renvoyée par `utiliserReducteur` vous permet de mettre à jour l'état avec une valeur différente et de déclencher un rendu. Le seul paramètre à passer à la fonction `dispatch` est l'action :
...
Réac définira le prochain état avec le résultat de l'appel de la fonction `reducer` que vous avez fournie avec le `state` courant et l'action que vous avez passée à `dispatch`.
...
* Si la nouvelle valeur fournie est identique au `state` actuel, déterminé par une comparaison avec [`Object.is`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/Object/is), Réac **sautera le nouveau rendu du composant et de ses enfants**. C'est une optimisation. Réac aura peut-être quand même besoin d'appeler votre composant avant d'en ignorer le résultat, mais ça ne devrait pas affecter votre code.

* Réac [met à jour l'état par lots](/learn/queueing-a-series-of-state-updates). Il met à jour l'écran **une fois que tous les gestionnaires d'événements ont été exécutés** et ont appelé leurs fonctions `set`. Ça évite les rendus multiples à la suite d'un événement unique. Dans les rares cas où vous devez forcer Réac à mettre à jour l'écran prématurément, par exemple pour accéder au DOM, vous pouvez utiliser [`flushSync`](/reference/Réac-dom/flushSync).

---

## Utilisation {/*usage*/}

### Ajouter un réducteur à un composant {/*adding-a-reducer-to-a-composant*/}

Appelez `utiliserReducteur` au niveau racine de votre composant pour gérer l'état avec un [réducteur](/learn/extracting-state-logic-into-a-reducer).
...
`utiliserReducteur` renvoie un tableau avec exactement deux éléments :
...
Réac passera l'état actuel et l'action à votre <CodeStep step={4}>fonction de réduction</CodeStep>. Votre réducteur calculera et renverra le nouvel état. Réac sauvera ce nouvel état, fera le rendu avec celui-ci puis mettra à jour l'interface utilisateur.
...
```js {5,9-12}
function Form() {
  const [state, dispatch] = utiliserReducteur(reducer, { name: 'Clara', age: 42 });
...
<Recipes titleText="Exemples simples de utiliserReducteur" titleId="examples-basic">
...
export default function Form() {
  const [state, dispatch] = utiliserReducteur(reducer, initialState);
...
Réac sauvegarde l'état initial une fois et l'ignore lors des rendus suivants.
...
function TodoList({ username }) {
  const [state, dispatch] = utiliserReducteur(reducer, createInitialState(username));
  // ...
```
...
Pour corriger ça, vous pouvez plutôt **la passer comme fonction _d'initialisation_** au `utiliserReducteur` en tant que troisième argument.
...
function TodoList({ username }) {
  const [state, dispatch] = utiliserReducteur(reducer, username, createInitialState);
  // ...
```
...
Dans l'exemple ci-dessus, `createInitialState` prend un argument `username`. Si votre fonction d'initialisation n'a besoin d'aucune information pour calculer l'état initial, vous pouvez passez `null` comme second argument à `utiliserReducteur`.
...
export default function TodoList({ username }) {
  const [state, dispatch] = utiliserReducteur(
    reducer,
    username,
    createInitialState
  );
...
En [Mode Strict](/reference/Réac/ModeStrict), Réac appellera votre réducteur et votre fonction d'initialisation deux fois. Ça ne devrait pas casser votre code.

Ce comportement **spécifique au développement** vous aide à [garder les composants purs](/learn/keeping-composants-pure). Réac utilise le résultat de l'un des appels et ignore l'autre. Tant que votre composant, votre fonction d'initialisation et votre réducteur sont purs, ça ne devrait pas affecter votre logique. Si toutefois ils sont malencontreusement impurs, ça vous permettra de détecter les erreurs.
...
Comme Réac appelle deux fois votre fonction de réduction, vous verrez que la liste a été ajoutée deux fois, vous saurez donc qu'il y a un problème. Dans cet exemple, vous pouvez le corriger [en remplaçant le tableau plutôt que de le modifier en place](/learn/updating-arrays-in-state#adding-to-an-array) :
...
Maintenant que cette fonction de réduction est pure, l'appeler une fois de plus ne change pas le comportement. C'est pourquoi Réac vous aide à détecter des erreurs en l'appelant deux fois. **Seuls les composants, les fonctions d'initialisation et les réducteurs doivent être purs.** Les gestionnaires d'événements n'ont pas besoin de l'être, Réac ne les appellera donc jamais deux fois.
