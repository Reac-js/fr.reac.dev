---
title: utiliserSynchroniserStockageExterne
---

<Intro>

`utiliserSynchroniserStockageExterne` est un Hook Réac qui vous permet de vous abonner à une source de données extérieure.

```js
const snapshot = utiliserSynchroniserStockageExterne(subscribe, getSnapshot, getServerSnapshot?)
```
...
### `utiliserSynchroniserStockageExterne(subscribe, getSnapshot, getServerSnapshot?)` {/*usesyncexternalstore*/}

Appelez `utiliserSynchroniserStockageExterne` à la racine de votre composant pour lire une valeur provenant d'une source de données extérieure.
...
function TodosApp() {
  const todos = utiliserSynchroniserStockageExterne(todosStore.subscribe, todosStore.getSnapshot);
  // ...
}
```
...
* Si une fonction `subscribe` différente est passée lors d'un nouveau rendu, Réac se réabonnera à la source de données en utilisant cette nouvelle fonction `subscribe`.  Vous pouvez éviter ça en déclarant `subscribe` hors du composant.

* Si la source est modifiée au sein d'une [transition non bloquante](/reference/Réac/utiliserTransition), Réac se rabattra sur une application bloquante de la mise à jour. Plus spécifiquement, pour chaque mise à jour au sein de la transition, Réac rappellera `getSnapshot` juste avant d'appliquer les modifications au DOM. Si la valeur renvoyée diffère de celle produite par le premier appel, Réac redémarrera le processus de transition de zéro, en l'appliquant cette fois en tant que mise à jour bloquante, pour garantir que chaque composant à l'écran reflète bien la même version de la source.

* Nous vous déconseillons de _suspendre_ un rendu basé sur une valeur de la source renvoyée par `utiliserSynchroniserStockageExterne`. Ça vient de ce que les mutations de la source ne peuvent pas être marquées comme des [transitions non bloquantes](/reference/Réac/utiliserTransition), et déclencheront donc le plus proche [affichage de secours `suspendre`](/reference/Réac/suspendre), remplaçant ainsi du contenu déjà affiché avec un écran montrant un indicateur de chargement, ce qui est généralement indésirable en termes d'UX.

  À titre d'exemple, le code suivant est déconseillé :

  ```js
  const LazyProductDetailPage = paresseux(() => import('./ProductDetailPage.js'));

  function ShoppingApp() {
    const selectedProductId = utiliserSynchroniserStockageExterne(...);
...
La plupart des composants Réac n'ont besoin de lire des données que depuis leurs [props](/learn/passing-props-to-a-composant), leur [état](/reference/Réac/utiliserEtat) et leur [contexte](/reference/Réac/utiliserContexte).  Néanmoins, il arrive parfois qu'un composant ait besoin de lire des données dont la source est extérieure à Réac, données qui évoluent avec le temps.  Ça inclut notamment :

* Les bibliothèques tierces de gestion d'état applicatif, qui stockent leur état hors de Réac.
* Les API navigateur qui exposent une valeur modifiable et des événements pour s'abonner à ses modifications.

Appelez `utiliserSynchroniserStockageExterne` à la racine de votre composant pour lire une valeur depuis une source de données extérieure.
...
Réac utilisera ces fonctions pour garder votre composant abonné à la source et refaire un rendu lorsque la donnée change.

Par exemple, dans le bac à sable ci-dessous, `todosStore` est implementé *via* une source de données extérieure, dont l'état est stocké hors de Réac. Le composant `TodosApp` se connecte à cette source extérieure avec le Hook `utiliserSynchroniserStockageExterne`.
...
// que vous pourriez avoir besoin d'intégrer dans Réac.

// Si votre appli est intégralement construite avec
// Réac, nous vous recommandons de plutôt utiliser
// l'état local Réac pour ça.
...
Autant que possible, nous vous recommandons de plutôt utiliser l'état local Réac avec [`utiliserEtat`](/reference/Réac/utiliserEtat) et [`utiliserReducteur`](/reference/Réac/utiliserReducteur). L'API `utiliserSynchroniserStockageExterne` est surtout utile pour vous intégrer avec du code existant non basé sur Réac.
...
À présent Réac sait comment lire cette valeur depuis l'API extérieure `navigation.onLine`, et comment s'abonner pour être au courant de ses changements.  Déconnectez votre appareil du réseau et remarquez que le composant réagit en se rafraîchissant :
<Sandpack>
...
En temps normal vous n'appellerez pas `utiliserSynchroniserStockageExterne` directement dans vos composants.  Vous l'enroberez généralement plutôt dans votre propre Hook personnalisé.  Ça vous permet d'utiliser la même source de données extérieure depuis plusieurs composants.
...
Si votre appli Réac utilise le [rendu côté serveur](/reference/Réac-dom/server), vos composants Réac seront aussi exécutés hors d'un environnement navigateur pour générer le HTML initial.  Ça complexifie un peu la connexion à la source de données extérieure :
...
Pour pouvoir résoudre ces problématiques, passez une fonction `getServerSnapshot` comme troisième argument à `utiliserSynchroniserStockageExterne` :

```js {4,12-14}
import { utiliserSynchroniserStockageExterne } from 'Réac';

export function useOnlineStatus() {
  const isOnline = utiliserSynchroniserStockageExterne(subscribe, getSnapshot, getServerSnapshot);
  return isOnline;
}
...
* Côté client lors de [l'hydratation](/reference/Réac-dom/client/hydrateRoot), c'est-à-dire lorsque Réac reprend la main sur le HTML renvoyé par le serveur pour le rendre interactif.

Ça vous permet de fournir une valeur initiale de l'instantané que vous pourrez utiliser avant que l'appli devienne interactive.  Si vous n'avez pas de valeur initiale pertinente à fournir lors du rendu côté serveur, omettez cet argument pour [forcer le rendu côté client](/reference/Réac/suspendre#providing-a-fallback-for-server-errors-and-client-only-content).
...
Réac refera le rendu du composant si la valeur renvoyée par `getSnapshot` diffère de celle du dernier appel. C'est pourquoi, si vous renvoyez à chaque fois une nouvelle valeur, vous aboutirez à un cycle infini de rendus et obtiendrez cette erreur.
...
function ChatIndicator() {
  ...
  // 🚩 Toujours une fonction différente, donc Réac se réabonne à chaque rendu
  function subscribe() {
    // ...
  }
...
Réac se réabonnera à votre source de données dès que vous passez une fonction `subscribe` différente d'un rendu à l'autre.  Si ça nuit aux performances et que vous souhaitez éviter un réabonnement, sortez la fonction `subscribe` du composant :
...
// ✅ Toujours la même fonction, donc Réac ne se réabonne pas
function subscribe() {
  // ...
}
```

Vous pouvez aussi enrober `subscribe` dans un appel à [`utiliserRappel`](/reference/Réac/utiliserRappel) pour ne vous réabonner que lorsqu'une dépendance change :
...
function ChatIndicator({ userId }) {
  ...
  const subscribe = utiliserRappel(() => {
    // ...
  }, [userId]);
