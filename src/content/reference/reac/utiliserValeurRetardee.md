---
title: utiliserValeurRetardee
---

<Intro>

`utiliserValeurRetardee` est un Hook Réac qui vous laisse différer la mise à jour d'une partie de l'interface utilisateur *(UI, NdT)*.

```js
const deferredValue = utiliserValeurRetardee(value)
```
...
### `utiliserValeurRetardee(value)` {/*usedeferredvalue*/}

Appelez `utiliserValeurRetardee` à la racine de votre composant pour recevoir une version différée de cette valeur.

```js
import { utiliserEtat, utiliserValeurRetardee } from 'Réac';
...
Durant le rendu initial, la valeur différée renvoyée sera celle que vous avez fournie. Lors des mises à jour, Réac tentera d'abord un rendu avec l'ancienne valeur (il va donc renvoyer l'ancienne valeur), et ensuite essayer en arrière-plan un rendu avec la nouvelle valeur (il va donc renvoyer la valeur à jour).

#### Limitations {/*caveats*/}

- Les valeurs que vous passez à `utiliserValeurRetardee` doivent être soit des valeurs primitives (comme des chaînes de caractères ou des nombres), soit des objets créés en-dehors du rendu. Si vous créez un nouvel objet pendant le rendu et que vous le passez immédiatement à `utiliserValeurRetardee`, il sera différent à chaque rendu, entraînant des rendus inutiles en arrière-plan.

- Quand `utiliserValeurRetardee` reçoit une valeur différente (en comparant au moyen de [`Object.is`](https://developer.mozilla.org/fr/docs/Web/JavaScript/Reference/Global_Objects/Object/is)), en plus du rendu en cours (dans lequel il utilisera encore la valeur précédente), il planifie un rendu supplémentaire en arrière-plan avec la nouvelle valeur. Ce rendu d'arrière-plan est susceptible d'être interrompu : s'il y a un nouvelle mise à jour de `value`, Réac le recommencera de zéro. Par exemple, si l'utilisateur tape dans un champ de saisie trop rapidement pour qu'un graphique basé sur sa valeur différée puisse suivre, le graphique ne se mettra à jour qu'une fois que l'utilisateur aura terminé sa saisie.

- `utiliserValeurRetardee` s'intègre très bien avec [`<suspendre>`](/reference/Réac/suspendre). Si la mise à jour d'arrière-plan suspend l'UI, l'utilisateur ne verra pas l'UI de secours : il continuera à voir l'ancienne valeur différée jusqu'à ce que les données soient chargées.

- `utiliserValeurRetardee` n'empêche pas par lui-même des requêtes réseau supplémentaires.

- `utiliserValeurRetardee` ne recourt pas à un différé de durée fixe. Dès que Réac termine le premier nouveau rendu, il commence immédiatement à travailler sur le rendu d'arrière-plan avec la nouvelle valeur différée. Toute mise à jour causée par des évènements (comme écrire dans un champ de saisie) interrompra le rendu d'arrière-plan et sera traitée en priorité.

- Le rendu d'arrière-plan entraîné par un `utiliserValeurRetardee` ne déclenche pas les Effets tant qu'il n'est pas retranscrit à l'écran. Si le rendu d'arrière-plan suspend, ses Effets ne seront lancés qu'après que les données seront chargées et que l'UI sera mise à jour.
...
Appelez `utiliserValeurRetardee` à la racine de votre composant pour différer la mise à jour de certaines parties de votre interface utilisateur.
...
Lors des mises à jour, la <CodeStep step={2}>valeur différée</CodeStep> sera « en retard » par rapport à la dernière <CodeStep step={1}>valeur</CodeStep>. Plus particulièrement, Réac fera d'abord un rendu *sans* mettre à jour la valeur différée, puis tentera un rendu supplémentaire en arrière-plan avec la nouvelle valeur reçue.

**Parcourons un exemple afin de comprendre l'utilité de ce Hook.**

<Note>

Cet exemple part du principe que vous utilisez une source de donnée compatible avec suspendre :
...
// qui n'est pas encore disponible dans les versions stables de Réac.

// Si vous souhaitez suivre cet exemple avec une version stable,
// essayez un framework intégrant suspendre, comme Relay ou Next.js.
...
// du framework avec lequel vous utilisez suspendre.
...
Une alternative visuelle courante consiste à *différer* la mise à jour d'une liste de résultats, en continuant à montrer les anciens résultats jusqu'à ce que les nouveaux soient prêts. Appelez `utiliserValeurRetardee` pour pouvoir passer une version différée de la recherche :
...
1. **Pour commencer, Réac refait un rendu avec la nouvelle `query` (`"ab"`) mais avec l'ancienne `deferredQuery` (toujours `"a")`.** La valeur `deferredQuery`, que vous passez à la liste de résultats, est *différée* : elle est « en retard » par rapport à la valeur `query`.

2. **En arrière-plan, Réac tente alors un autre rendu avec `query` et `deferredQuery` valant *toutes les deux* `"ab"`.** Si ce rendu aboutit, Réac l'affichera à l'écran. Cependant, s'il suspend (les résultats pour `"ab"` ne sont pas encore chargés), Réac abandonnera cet essai de rendu, et essaiera à nouveau une fois les données chargées. L'utilisateur continuera à voir l'ancienne valeur différée jusqu'à ce que les données soient prêtes.

Le rendu différé « d'arrière-plan » est susceptible d'être interrompu. Par exemple, si vous tapez à nouveau dans le champ de saisie, Réac l'abandonnera et recommencera avec la nouvelle valeur. Réac utilisera toujours la dernière valeur fournie.
...
Vous pouvez également utiliser `utiliserValeurRetardee` pour optimiser les performances. C'est pratique lorsqu'une partie de votre UI a un rendu lent, qu'il n'y a pas de manière simple de l'optimiser, et que vous voulez éviter qu'elle bloque le reste de l'UI.
...
Pour commencer, optimisez `SlowList` pour éviter un nouveau rendu quand ses propriétés n'ont pas changé. Pour ce faire, [enrobez-le avec `memoire`](/reference/Réac/memoire#skipping-re-rendering-when-props-are-unchanged) :

```js {1,3}
const SlowList = memoire(function SlowList({ text }) {
  // ...
});
```
...
Concrètement, le souci de performances principal vient de ce que lorsque vous tapez dans le champ de saisie, la `SlowList` reçoit des nouvelles propriétés, et la lenteur de sa mise à jour rend la saisie saccadée. Dans un tel cas, `utiliserValeurRetardee` vous permet de prioriser la mise à jour du champ de saisie (qui doit être rapide) par rapport à celle de la liste de résultats (qui peut être plus lente) :

```js {3,7}
function App() {
  const [text, setText] = utiliserEtat('');
  const deferredText = utiliserValeurRetardee(text);
  return (
    <>
      <input value={text} onChange={e => setText(e.target.value)} />
...
Ça n'accélère pas le rendu de `SlowList`. Néanmoins, ça indique à Réac de déprioriser le rendu de la liste afin de ne pas bloquer les frappes clavier. La liste sera « en retard » par rapport au champ de saisie, pour finalement le « rattraper ». Comme auparavant, Réac essaiera de mettre à jour la liste le plus vite possible, mais sans empêcher l'utilisateur de taper.

<Recipes titleText="Différence entre utiliserValeurRetardee et un rendu non optimisé" titleId="examples">

#### Différer le rendu de la liste {/*deferred-re-rendering-of-the-list*/}

Dans cet exemple, chaque élément du composant `SlowList` est **artificiellement ralenti** afin que vous puissiez constater que `utiliserValeurRetardee` permet de garder le champ de saisie réactif. Écrivez dans le champ de saisie et voyez comme la saisie reste réactive, alors que la liste « est en retard ».
...
Dans cet exemple, chaque élément du composant `SlowList` est **artificiellement ralenti**, mais on n'utilise pas `utiliserValeurRetardee`.

Vous pouvez remarquer que la frappe dans le champ de saisie est très saccadée. Sans `utiliserValeurRetardee`, chaque frappe clavier force la liste entière à refaire un rendu immédiatement, sans possibilité d'interruption.
...
Cette optimisation nécessite que `SlowList` soit enrobée par un [`memoire`](/reference/Réac/memoire). En effet, à chaque fois que `text` change, Réac doit pouvoir refaire le rendu du composant parent rapidement. Durant ce rendu, `deferredText` a toujours sa valeur précédente, donc `SlowList` peut s'épargner un nouveau rendu (ses propriétés n'ont pas changé). Sans [`memoire`](/reference/Réac/memoire), il y aurait un nouveau rendu dans tous les cas, ce qui tue tout l'intérêt de l'optimisation.
...
Mêmes si ces techniques sont utiles dans certains cas, `utiliserValeurRetardee` est plus adapté pour optimiser le rendu, car il est totalement intégré avec Réac et il s'adapte à l'appareil de l'utilisateur.
...
De plus, les rendus différés planifiés par `utiliserValeurRetardee` sont par défaut susceptibles d'être interrompus, ce qui n'est pas le cas du *debouncing* ou du *throttling*. Ça signifie que si Réac est en plein milieu du rendu d'une vaste liste, et que l'utilisateur ajuste sa saisie, Réac abandonnera ce rendu, traitera la frappe, et recommencera le rendu en arrière-plan. Par opposition, le *debouncing* et le *throttling* donneraient ici toujours une expérience saccadée car ils sont *bloquants* : ils diffèrent simplement le moment auquel le rendu bloque la frappe.
